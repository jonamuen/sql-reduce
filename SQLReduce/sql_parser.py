...
class SQLParser(Lark):
    ...
    Uses a SQL-parser which recognizes certain SQL statements and a backup parser
    for statements not recognized by the first parser.
    """
    ...
    def parse(self, text: str, start=None) -> NamedTree:
        ...
        which are then parsed one at a time. If an error occurs, the
        affected statement is parsed by the backup parser (unrecognized_stmt_parser).
