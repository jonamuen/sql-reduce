...
from SQLReduce.utils import partial_equivalence, get_grammar, PrettyPrinter, split_into_stmts
from SQLReduce.reductions import StatementRemover, ColumnRemover, ExprSimplifier, \
    ...
    StatementRemoverByType, CaseSimplifier, ConstraintRemover, UnrecognizedExprRemover, ListItemRemover, \
    ValueMinimizer
...
from SQLReduce.verifier import AbstractVerifier, SQLiteReturnSetVerifier, Verifier, DuckDBVerifier
...
class StatementSplittingTest(unittest.TestCase):
    """Test the split_into_stmts function"""
    def test_semi_in_str(self):
        text = "SELECT ';';"
        self.assertEqual(["SELECT ';';"], list(split_into_stmts(text)))

    def test_multiple(self):
        text = "SELECT ';'; SELECT 4; SELECT '\n;';"
        self.assertEqual(["SELECT ';';", " SELECT 4;", " SELECT '\n;';"], list(split_into_stmts(text)))
...
class ParserTest(unittest.TestCase):
    ...
    def test_min_max(self):
        tree = self.parser.parse("SELECT MIN(c0), MAX(c1) FROM t0;")
        self.assertEqual(1, len(list(tree.find_data('k_min'))))
        self.assertEqual(1, len(list(tree.find_data('k_max'))))
        self.assertEqual(0, len(list(tree.find_data('unexpected_stmt'))))
...
class SQLSmithParserTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        for f in self.sqlitedir.iterdir():
            if f.suffix == '.sql':
                ...
                try:
                    ...
                    logging.error(cmd)
                    raise e
        logging.info(f"recognized/passed/total: {recognized}/{passed}/{total}")
...
class UnrecognizedExprRemoverTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.parser = Lark(grammars.unrecognized, start='unexpected_stmt', debug=True, parser='lalr')

    def test_all_transforms(self):
        stmt = 'SELECT (c0 + (c2 - 1)) * (3 + c1) FROM t0;'
        tree = self.parser.parse(stmt)
        node_remover = UnrecognizedExprRemover(multi_remove=False)
        self.assertEqual(6, len(list(node_remover.all_transforms(tree))))
        node_remover = UnrecognizedExprRemover(multi_remove=True)
        self.assertEqual(9, len(list(node_remover.all_transforms(tree))))

    def test_simple(self):
        stmt = 'SELECT (c0 + 2) * 3 FROM t0;'
        tree = self.parser.parse(stmt)
        node_remover = UnrecognizedExprRemover(multi_remove=False)
        expected = {'SELECT 3 FROM t0;', '* 3 FROM t0;'}
        self.assertEqual(expected, set(map(lambda x: PrettyPrinter().transform(x[1]), node_remover.all_transforms(tree))))
...
class ExprSimplifierTest(unittest.TestCase):
    ...
    def test_1(self):
        ...
        self.assertIn('SELECT c0 FROM t0;', results)
        self.assertIn('SELECT NOT c1 * c2 FROM t0;', results)

    def test_2(self):
        tree = self.parser.parse("SELECT NULLIF(c0 + c1, c0 - c1) FROM t0;")
        results = set(map(lambda x: PrettyPrinter().transform(x[1]), self.simplifier.all_transforms(tree)))
        self.assertIn('SELECT (c0 + c1) FROM t0;', results)
        self.assertIn('SELECT (c0 - c1) FROM t0;', results)
    ...
    def test_join(self):
        tree = self.parser.parse("SELECT c0 FROM t0 FULL OUTER JOIN t1;")
        results = set(map(lambda x: PrettyPrinter().transform(x[1]), self.simplifier.all_transforms(tree)))
        self.assertIn('SELECT c0 FROM t0;', results)
...
class DuckDBVerifierTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        with open('test/real_testcases/duckdb/test.sql') as f:
            stmt = f.read()
...
class ReducerTest(unittest.TestCase):
    ...
    def test_unneeded_inserts_and_tables(self):
        ...
        verifier = SQLiteReturnSetVerifier(list(split_into_stmts(stmt)))
        reducer = Reducer(self.parser, verifier, [StatementRemover(), ColumnRemover(), ExprSimplifier()], [], output='/dev/null')
        result = reducer.reduce(stmt)
        self.assertEqual(expected, result)
        self.assertTrue(verifier.verify(list(split_into_stmts(result))))
    ...
    def test_all(self):
        # mixed case causes a parse error -> forces use of the backup parser
        stmt = "CREATE TABLE IF NOT EXISTS t0(c0 INT, c1 VARCHAR(16));\n" \
               "CREATE TABLE unused(c0 INT);\n" \
               "DELETE FROM t0;\n" \
               "INSERT INTO t0(c0, c1) VALUES (0, NULL);\n" \
               "INsERT INTO t0(c0, c1) VALUES (1, NULL);\n" \
               "InSeRt INTO t0(c0) VALUES (2), (3);\n" \
               "SELECT NOT(NOT(t0.c0)), (((4))) FROM t0 LEFT OUTER JOIN unused;"

        sql_grammar = get_grammar(grammars.sql, grammars.lark)
        optionals = OptionalFinder().transform(sql_grammar)
        reduction_passes = [StatementRemoverByType(), StatementRemover(), OptionalRemover(optionals=optionals),
                            CompoundSimplifier(), ColumnRemover(), ExprSimplifier(), ListItemRemover(),
                            BalancedParenRemover(), UnrecognizedExprRemover(), TokenRemover(), TokenRemoverNonConsec()]
        verifier = SQLiteReturnSetVerifier(list(split_into_stmts(stmt)))
        reducer = Reducer(self.parser, verifier, reduction_passes, [], output='/dev/null')

        result = reducer.reduce(stmt)

        expected = "CREATE TABLE t0 (c0);\n" \
                   "INSERT INTO t0 VALUES (0);\n" \
                   "InSeRt INTO t0 VALUES (2);\n" \
                   "SELECT NOT c0, 4 FROM t0;"

        self.assertEqual(expected, result)
        self.assertTrue(verifier.verify(list(split_into_stmts(result))))


# These tests aren't really tests, but rather a showcase of SQL-Reduce applied to real bugs discovered by
# Manuel Rigger in various versions of DuckDB.
# Many don't even include assertions and thus will pass as long as SQL-Reduce doesn't crash.
# All the test should work, provided that a debug build of duckdb_cli (version ed92e9c55) is available on the path, i.e.
# run 'export PATH=/path/to/duckdb/build/debug/:$PATH' before the tests.
# Since they take relatively long to run, we skip them.
@unittest.skip
class DuckDBReducerTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        optionals = OptionalFinder().transform(get_grammar(grammars.sql, grammars.lark))
        cls.parser = SQLParser('sql.lark', start="sql_stmt_list", debug=False, parser='lalr')
        cls.reduction_passes = [StatementRemoverByType(), StatementRemover(), CompoundSimplifier(), ColumnRemover(),
    ...
    def test_duckdb_issue3(self):
        # requires 4ad62e9 or ed92e9c55 (debug)
        with open('test/real_testcases/duckdb/issue3.sql') as f:
            stmt = f.read()
        reducer = Reducer(self.parser, Verifier('test/real_testcases/duckdb/test_3.py', output_name='test.sql'),
                          self.reduction_passes, self.canonicalizations, output='/dev/null')
        result = reducer.reduce(stmt)
        print(result)
        self.assertLessEqual(len(result), 136)

    def test_duckdb_issue4(self):
        # requires 340d867
        with open('test/real_testcases/duckdb/issue4.sql') as f:
            stmt = f.read()
        reducer = Reducer(self.parser, Verifier('test/real_testcases/duckdb/test_asan_segv.py', output_name='test.sql'),
                          self.reduction_passes, self.canonicalizations, output='/dev/null')
        result = reducer.reduce(stmt)
        print(result)
    ...
    def test_duckdb_issue6(self):
        # requires ed92e9c55 debug build
        with open('test/real_testcases/duckdb/issue6.sql') as f:
            stmt = f.read()
        reducer = Reducer(self.parser, Verifier('test/real_testcases/duckdb/test_asan_segv.py', output_name='test.sql'),
                          self.reduction_passes, self.canonicalizations, output='/dev/null')
        print(reducer.reduce(stmt))

    def test_duckdb_issue7(self):
        # requires 3f0eb51 (debug ed92e9c55 also works)
    ...
    def test_duckdb_issue8(self):
        # requires debug build f3c7d39 to reproduce (debug ed92e9c55 also works)
        with open('test/real_testcases/duckdb/issue8.sql') as f:
            stmt = f.read()
        reducer = Reducer(self.parser, Verifier('test/real_testcases/duckdb/test_8.py', output_name='test.sql'),
                          self.reduction_passes, self.canonicalizations, output='/dev/null')
        print(reducer.reduce(stmt))
